...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    icon: "🩺",
    items: [
      { key: "adult-emergency", label: "Adult Emergency Triage", icon: "🚑", mode: "Triage", triageType: "Adult emergency triage" },
      { key: "central-triage", label: "Central Triage", icon: "🩺", mode: "Triage", triageType: "Central Triage" },
      { key: "pedi-triage", label: "Pedi Triage", icon: "👶", mode: "Triage", triageType: "Pedi Triage" },
      { key: "gyn-triage", label: "Gyn Triage", icon: "👩", mode: "Triage", triageType: "Gyn Triage" },
      { key: "obs-triage", label: "Obs Triage", icon: "🤰", mode: "Triage", triageType: "Obs Triage" },
      { key: "neonatal-triage", label: "Neonatal Triage", icon: "🍼", mode: "Triage", triageType: "Neonatal triage" },
    ],
  },
];
...
function App() {
  ...
  useEffect(() => {
    ...
    function closeTriageRegistrationMenu(event) {
      if (!event.target.closest?.("[data-triage-registration-dropdown]")) {
        setTriageRegMenuOpen(false);
        setTriageRegSubmenu(null);
      }
    }
    document.addEventListener("mousedown", closeTriageRegistrationMenu);
    return () => {
      subscription.unsubscribe();
      document.removeEventListener("mousedown", closeTriageRegistrationMenu);
    };
  }, []);
        ...
        <nav style={styles.topNav}>
          ...
          <div style={styles.navDropdown} data-triage-registration-dropdown>
            <button
              type="button"
              style={
                page === "Triage & Registration" || triageRegMenuOpen
            ...
            >
              🧾 Triage & Registration <span style={{ fontSize: 18 }}>{triageRegMenuOpen ? "▴" : "▾"}</span>
            </button>
              ...
              <>
                ...
                <div style={styles.overlayDropdown} role="menu">
                  <div style={styles.overlayDropdownTitle}>
                    <strong>🧾 Triage & Registration</strong>
                    <button
                      type="button"
          ...
          <TopNavButton page={page} setPage={setPage} name="Laboratory" icon="🧪" />
          <TopNavButton page={page} setPage={setPage} name="Pharmacy" icon="💊" />
          <TopNavButton page={page} setPage={setPage} name="Billing" icon="💰" />
          <TopNavButton page={page} setPage={setPage} name="Diagnostics" icon="🔬" />
          <TopNavButton page={page} setPage={setPage} name="Emergency" icon="🚑" />
          <TopNavButton page={page} setPage={setPage} name="Inpatient" icon="🛏️" />
          <TopNavButton page={page} setPage={setPage} name="OR" icon="🏥" />
          <TopNavButton page={page} setPage={setPage} name="Liaison" icon="🤝" />
          <TopNavButton page={page} setPage={setPage} name="QI & Report" icon="📊" />
        </nav>
      </aside>
      ...
      <main style={styles.main}>
        ...
        {page === "Dashboard" && (
          <Dashboard
            ...
            goTriage={() => { setTriageRegistrationMode("Triage"); setPage("Triage & Registration"); }}
            goDoctor={() => setPage("Doctor Consultation")}
          />
        )}

        {page === "Triage & Registration" && (
          <TriageRegistration
            user={session.user}
        ...
        {page === "OPD" && <OPD onMessage={(message) => alert(message)} />}
        ...
        {page === "Diagnostics" && (
          <Diagnostics user={session.user} />
        )}

        {["Emergency", "Inpatient", "OR", "Liaison", "QI & Report"].includes(page) && (
          <HospitalModule
            module={page}
            user={session.user}
          />
